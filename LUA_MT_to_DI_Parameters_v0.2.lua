...
Created by Georgi Dimitrov for dynamic trunk parameter configuration.
--]]
...
local locationPrefixDefault = scriptParameters.getValue("LocationPrefixDefault") or ""
...
function M.inbound_INVITE(msg)
    trace.format("--- Running Georgi's script - dynamic mapping with 6 LocationPrefixMap  ---")
    ...
    if locationHeader then
        ...
        if extractedId then
            ...
            if numberPrefix then
                ...
                -- Check if LocationPrefixDefault is configured as fallback
                if locationPrefixDefault ~= "" then
                    trace.format("Using LocationPrefixDefault as fallback: %s", locationPrefixDefault)
                    ...
                    local new_calledNumber = string.gsub(calledNumber, "%+", locationPrefixDefault)
                    local new_ruri = string.gsub(ruri, calledNumber, new_calledNumber, 1)

                    msg:setRequestUri(new_ruri)
                    trace.format("New Request URI set using default prefix to: %s", tostring(new_ruri))
                else
                    trace.format("LocationPrefixDefault is not configured. No number transformation applied.")
                end
            end
        else
